...
# the trend of a day
ggplot(sunlab_pv)+geom_boxplot(aes(x=as.factor(Hour),y=A_Horizontal...Power.DC..W.))
ggplot(sunlab_pv)+geom_boxplot(aes(x=as.factor(Hour),y=A_Horizontal...Current.DC..A.))
ggplot(sunlab_pv)+geom_boxplot(aes(x=as.factor(Hour),y=A_Horizontal...Voltage.DC..V.))
...
ggplot(sunlab_pv)+geom_boxplot(aes(x=as.factor(Month),y=A_Horizontal...Voltage.DC..V.))
ggplot(sunlab_pv)+geom_boxplot(aes(x=as.factor(Month),y=A_Horizontal...Current.DC..A.))
...
sunlab_A$Month <- month(sunlab_A$Datetime)
...
for(i in seq_along(file_number)){
  k<-(i+2013)
  file_name=paste("sunlab-faro-pv-",k,".csv",sep="")
  data<-read.csv(file=file_name, stringsAsFactors = FALSE, sep=';')
  df<-rbind(df,data)
}
...
sunlab_model <- lm(A_Optimal...Power.DC..W. ~ 
                     Month+MDay+Hour+Minute+Year+Optimal_Temperature,
                   data=sunlab_A)
...
sunlab_ridge<- sunlab_A[,c(2:6,17,12)]
